...
def generate_linear_model(training_data, latent_states, em_iter):
    """This is an extension of the simple Kalman FIlter Library to allow to 
    have multiple training examples"""
    time, dims,trials = np.shape(training_data)
    linear_state_model=kl.KalmanFilter()
    linear_state_model.n_dim_state = latent_states #Numb of states
    linear_state_model.n_dim_obs = dims #Numb of dimensions
    linear_state_model._initialize_parameters()
    linear_state_model.em(training_data[:,:,0], em_vars = 'all', n_iter = 1)#Initialize everything with one em iterations
    for idx in range(em_iter):#run em 
        main_struct=GlobalVariables(training_data, linear_state_model)
        obs_matrices,obs_covariance,trans_matrix,trans_cov,init_st_mean,init_st_cov,trans_off,obs_off=_em_main_loop(linear_state_model,main_struct,training_data)
        linear_state_model.observation_matrices=obs_matrices
        linear_state_model.transition_matrices=trans_matrix
        linear_state_model.transition_offsets=trans_off
        linear_state_model.observation_offsets=obs_off
        linear_state_model.transition_covariance=trans_cov
        linear_state_model.observation_covariance=obs_covariance
        #print kfUp.observation_matrices
        #print kfUp.observation_offsets
        linear_state_model.initial_state_mean=init_st_mean
        linear_state_model.initial_state_covariance=init_st_cov
    return linear_state_model
